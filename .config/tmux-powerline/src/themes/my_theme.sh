...
if [ -z $TMUX_POWERLINE_WINDOW_STATUS_CURRENT ]; then
	TMUX_POWERLINE_WINDOW_STATUS_CURRENT=(
		"#[$(format inverse)]" \
		"#[bg=2]$TMUX_POWERLINE_DEFAULT_LEFTSIDE_SEPARATOR" \
		" #[bold, bg=2]#I " \
		"#[bg=2]$TMUX_POWERLINE_SEPARATOR_RIGHT_THIN" \
		" #[bold, bg=2]#W " \
		"#[$(format regular)]" \
		"#[fg=2]$TMUX_POWERLINE_DEFAULT_LEFTSIDE_SEPARATOR"
	)
fi
...
if [ -z $TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS ]; then
	TMUX_POWERLINE_RIGHT_STATUS_SEGMENTS=(
		...
		#"xkb_layout 125 117" \
		"date_day 0 15" \
		"date 0 15 ${TMUX_POWERLINE_SEPARATOR_LEFT_THIN}" \
